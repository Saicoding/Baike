...
class Bookzgrm():
	...
	# 导航到初始搜索ID的页面
	def go_to_start_page(self, start_id):
		self.driver.get('http://www.crup.com.cn/Book/List')
		# 等待跳转输入框出现
		WebDriverWait(self.driver, 15, 0.5).until(EC.presence_of_element_located((By.CLASS_NAME, 'txt-page')))

		# 在页面跳转框输入数字
		self.driver.find_element(By.CLASS_NAME, 'txt-page').send_keys(start_id)

		# 找到跳转按钮
		self.driver.find_element_by_xpath('/html/body/div[1]/div[4]/div[3]/div[2]/div[3]/button').click()
		sleep(5)

	# 得到初始搜索ID
	def get_start_page_num(self, create_funs):
		result = create_funs.open_ini_result_object()
		return result['book_zgrm_start_id']

	# 导航到详情页
	def go_detail_page(self, create_funs):
		result = create_funs.open_ini_result_object()
		book_index = result['book_index']
		elements = self.driver.find_elements_by_css_selector('.list > li > a ')
		element = elements[book_index-1]
		element.click()
		sleep(5)
	...
	def get_result(self):
		...
		# 获取目录内容
		directory = self.driver.find_elements_by_css_selector('#textbook-con > div.textbook-right.fl > div.directory > div:nth-child(2)')[0].text
		directory = re.sub('(https|http):.*(com|cn|de)', '', directory)

		# 获取下载连接
		down_url = self.driver.find_element_by_css_selector('#textbook-con > div.textbook-left.fl > div.book-transform > p > a').get_attribute('href')

		return {'title': title, 'bar': bar, 'author': author, 'content': content, 'time': time, 'url':url,'author_info':author_info,'directory':directory,'down_url':down_url,'primitive_title':primitive_title}
...
class BasicBar():
	...
	def get_basic_str(self):
		bar = [self.zuozhe, self.shuhao, self.dingjia, self.zishu, self.yinci, self.kaiben, self.chubanshijian, self.ISBN, self.baozhuang]
		return bar
